...
describe('01 - Login', () => {
  ...
  it('permite iniciar sesión como almacenero1 (rol ALMACENERO de Casa Matriz)', () => {
    cy.visit('/login');
    cy.get('input[name="username"]').type('almacenero1');
    cy.get('input[name="password"]').type('Almacen123!');
    cy.contains('button[type="submit"]', 'Ingresar al sistema').click();
    cy.wait(700);

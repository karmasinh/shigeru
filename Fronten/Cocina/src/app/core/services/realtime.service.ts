...
import { PedidoEvento } from '../models';
...
export class RealtimeService {
  ...
  pedidosSucursal$(sucursalId: number): Observable<PedidoEvento> {
    return new Observable(subscriber => {
      ...
      const suscribir = () => {
        stompSub = client.subscribe(`/topic/pedidos/${sucursalId}`, (msg: IMessage) => {
          try { subscriber.next(JSON.parse(msg.body)); } catch { /* payload no parseable, se ignora */ }
        });
      };

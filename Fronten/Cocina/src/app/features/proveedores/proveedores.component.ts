  ...
  template: `
      ...
      <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p class="text-sm" style="color:rgb(var(--color-on-surface)/0.5)">
            {{ proveedores().length }} registros · Gestión de proveedores de insumos y materias primas
          </p>
        </div>
        <div class="flex gap-2 w-full sm:w-auto">
          <input [ngModel]="busqueda()" (ngModelChange)="busqueda.set($event); pagina.set(1)"
                 class="input w-full sm:w-52 text-sm"
                 placeholder="Buscar proveedor..." maxlength="100">
          ...
          <table>
            <thead>
              <tr>
                ...
                <th class="cursor-pointer select-none" (click)="sortBy('nit')">
                  NIT {{ si('nit') }}
                </th>
                <th class="cursor-pointer select-none" (click)="sortBy('contacto')">
              ...
              @if (!cargando() && filtrados().length === 0) {
                <tr>
                  <td colspan="6" class="text-center py-12" style="color:rgb(var(--color-on-surface)/0.35)">
                    <iconify-icon icon="tabler:building-store" width="32" height="32" style="color:currentColor" class="mb-2 inline-block"></iconify-icon>
    ...
    @if (modal()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
           (click)="cerrar()">
        <div class="card max-w-lg w-full space-y-4 animate-fade-up" (click)="$event.stopPropagation()">
          ...
          <div class="flex items-center justify-between">
            <h3 class="font-display font-bold text-lg inline-flex items-center gap-1.5" style="color:rgb(var(--color-on-surface))">
              @if (editandoId()) {
                ...
                <iconify-icon icon="tabler:building-store" width="18" height="18" style="color:currentColor"></iconify-icon> Nuevo Proveedor
              }
            </h3>
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input [(ngModel)]="form.correo" class="input text-sm"
                     type="email" placeholder="contacto@proveedor.com" maxlength="150">
            </div>
            <div>
...
export class ProveedoresComponent implements OnInit {
  ...
  filtrados = computed(() => {
    ...
    let lista = !q ? this.proveedores() : this.proveedores().filter(p =>
  ...
  abrirEditar(p: Proveedor): void {
    ...
    this.form = { nombre: p.nombre, nit: p.nit, telefono: p.telefono ?? '', correo: p.correo ?? '', contacto: p.contacto ?? '', direccion: p.direccion ?? '' };
    this.error.set('');
    this.modal.set(true);
  }
    ...
    obs.subscribe({
      next: p => {
        ...
        this.proveedores.update(list =>
          id ? list.map(x => x.id === id ? p : x) : [...list, p]
        );

      ...
      <div class="ent-panel-right">
        ...
        <div class="ent-form-container">
          ...
          <p class="ent-form-subtitle">
            Acceso al módulo de cocina,<br>
            ingresa tus credenciales para continuar.
          </p>

          @if (bloqueado()) {
            <div class="ent-alert-blocked">
              <span class="inline-flex items-center"><iconify-icon icon="tabler:lock" width="20" height="20" style="color:currentColor"></iconify-icon></span>
              <div>
                <p class="ent-alert-title">Usuario bloqueado</p>
          ...
          <form (ngSubmit)="onLogin()" style="margin-bottom: 0;">
            ...
            <div class="ent-field">
              <div class="ent-input-wrapper">
                ...
                <input [(ngModel)]="form.username" name="username"
                       ...
                       autocomplete="username" autocapitalize="off" [disabled]="loading()" required>
              </div>
            </div>
            ...
            @if (error()) {
              <div class="ent-error inline-flex items-center gap-1">
          ...
          <!-- Switch sistema -->
          <a [href]="ventasUrl" class="ent-switch-link">
...
export class LoginComponent {
  ...
  ventasUrl    = (environment as any).ventasUrl ?? 'http://localhost:4200';
  ...
  onLogin(): void {
    if (!this.form.username || !this.form.password) {
      this.error.set('Complete usuario y contraseña.');
      return;
    }
    ...
    this.authService.login(this.form).subscribe({
      ...
      error: (err) => {
        ...
        const msg = err?.error?.mensaje ?? err?.message ?? 'Error al iniciar sesión';
        this.error.set(msg);

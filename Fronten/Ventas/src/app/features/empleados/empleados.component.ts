        ...
        <table>
          <thead>
            <tr>
              ...
              <th (click)="sortBy('ci')" class="cursor-pointer select-none">
                <div class="flex items-center gap-1">CI <span class="text-xs opacity-40">{{ si('ci') }}</span></div>
              </th>
              <th (click)="sortBy('cargo')" class="cursor-pointer select-none">
                <div class="flex items-center gap-1">Cargo <span class="text-xs opacity-40">{{ si('cargo') }}</span></div>
              </th>
              <th (click)="sortBy('turno')" class="cursor-pointer select-none">
                <div class="flex items-center gap-1">Turno <span class="text-xs opacity-40">{{ si('turno') }}</span></div>
              </th>
          ...
          <div class="flex items-center justify-between mb-5">
            <h3 class="font-display font-bold text-lg inline-flex items-center gap-1.5"
              ...
              @if (editando()) {
                ...
                <iconify-icon icon="line-md:account" width="18" height="18" style="color:currentColor"></iconify-icon>
                Nuevo empleado
              }
            </h3>
    ...
    @if (modalRol()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="card max-w-sm w-full animate-fade-up"
             (click)="$event.stopPropagation()">
          <h3 class="font-display font-bold mb-4 inline-flex items-center gap-1.5"
              style="color:rgb(var(--color-on-surface))">
...
export class EmpleadosComponent implements OnInit {
  ...
  cargarDatos(): void {
    ...
    this.http.get<any[]>(`${environment.apiUrl}/empleados`).subscribe({
  ...
  confirmarCambioRol(rolId: number): void {
    ...
    if (!emp?.usuarioId) {
      this.toastSvc.error('Este empleado no tiene usuario asignado');

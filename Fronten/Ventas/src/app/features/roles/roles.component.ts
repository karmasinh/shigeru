  ...
  template: `
      ...
      <div class="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p class="text-sm" style="color:rgb(var(--color-on-surface)/0.5)">
            {{ roles().length }} registros · Define roles y asigna módulos de acceso
          </p>
        </div>
      ...
      <div class="grid gap-3">
        @if (cargando()) {
          @for (i of [1,2,3]; track i) {
            <div class="card h-24" style="animation:pulse 1.5s infinite"></div>
          }
        }
    ...
    @if (modal()) {
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="card max-w-lg w-full space-y-4 animate-pop"
          ...
          <div class="space-y-3">
            <div>
              ...
              <input [(ngModel)]="form.nombre" class="input text-sm"
                     placeholder="Ej: CAJERO, GERENTE_SUCURSAL" maxlength="80">
            </div>
            <div>
            ...
            <div>
              <label class="input-label mb-2 block">
                Módulos de acceso
                <span class="font-normal" style="color:rgb(var(--color-on-surface)/0.4)">
                  ({{ modulosSeleccionados().size }} seleccionados)
                </span>
              </label>
              @if (cargandoModulos()) {
                <div class="skeleton h-32 rounded-xl"></div>
              } @else {
                <div class="rounded-xl border p-3 space-y-1 overflow-y-auto"
                     style="border-color:rgb(var(--color-border));max-height:200px;background:rgb(var(--color-surface))">
                  @for (mod of modulos(); track mod.id) {
                    <label class="flex items-center gap-2 text-xs cursor-pointer select-none py-1"
                      ...
                      <input type="checkbox"
                             [checked]="modulosSeleccionados().has(mod.id)"
